...
type
  ...
  TframeAccPeriods = class(TFrame)
    ...
    procedure accEntrysDrawColumnCell(Sender: TObject; const Rect: TRect;
    ...
    procedure accEntrysSelectEditor(Sender: TObject; Column: TColumn;
      var Editor: TWinControl);
    procedure dbChkBoxCloseClick(Sender: TObject);
    procedure dbEdtPerNameKeyPress(Sender: TObject; var Key: char);
    procedure dtEdtEndAcceptDate(Sender: TObject; var ADate: TDateTime;
  ...
  private
    ...
    function   verifyRecord(const ds: TDataset;
                          const verifName: boolean = True): boolean;
 ...
 if Tcheckbox(Sender).Focused and (Tcheckbox(Sender).tag=-1) then
  ...
  if Tcheckbox(Sender).Checked then
    ...
    if  (qryAccPeriods.RecordCount>0) and (dialogs.messageDlg(estbk_strmsg.SCNumeratorConversions,mtConfirmation,[mbYes,mbNo],0)<>mrYes) then
       begin
         // 25.01.2013 Ingmar; üks kasutaja suutis leida bugi, mille peale ei mõelnudki Rasmus Põld
         // Ta vajutas muudame numeraatorid rp. aasta põhiseks, siis vastas NO ja siis avas uus rp. aasta
         // ning koostati täiesti bogus numeraatorid !!!!!!
...
begin
  ...
  if DataSet.Active and not FSkipScrollEvent then
    try
      ...
      // --- tobe, aga peame eelmise perioodi lõpu ka teadma saama; meil alati DESC ORDER
      ...
      begin
        ...
        begin
          ...
          if dateutils.DaysBetween(DataSet.FieldByName('period_start').AsDateTime,dateutils.StartOfAYear(YearOf(DataSet.FieldByName('period_start').AsDateTime)))=0 then
...
//      pBtn.Caption:='...';
      ...
      dtEdtEnd.Enabled   := self.FNewRecordCreated or
...
procedure TframeAccPeriods.dtEdtEndAcceptDate(Sender: TObject;
...
function TframeAccPeriods.verifyRecord(const ds: TDataset;
  const verifName: boolean = True): boolean;
begin
  Result := False;
  with ds do
  begin
    ...
    if not FieldByName('period_end').isNull and
      (FieldByName('period_start').AsDateTime > FieldByName('period_end').AsDateTime) then
    begin
    ...
    if (ds.RecNo <> ds.RecordCount) and
       (dateutils.DaysBetween(FieldByName('period_start').AsDateTime,dateutils.incDay(self.FPriorEndDate, 1))<>0) then
//      (FieldByName('period_start').AsDateTime <> dateutils.incDay(self.FPriorEndDate, 1)) then
    begin
         //showmessage(inttostr(dateutils.DaysBetween(FieldByName('period_start').AsDateTime,dateutils.incDay(self.FPriorEndDate, 1))) +
         //datetimetostr(FieldByName('period_start').AsDateTime)+' '+datetimetostr(dateutils.incDay(self.FPriorEndDate, 1)));
  ...
  {
  Küsimus selline, et miks ei saa lisada majandusaastat, mis on pikem kui 12 kuud? Asutamisel nt võib majandusaasta pikkuseks
olla kuni 18 kuud.
}

    if not FieldByName('period_end').isNull and
      (dateUtils.MonthsBetween(FieldByName('period_end').AsDateTime,
...
procedure TframeAccPeriods.accEntrysDrawColumnCell(Sender: TObject;
...
begin
  // showmessage('test '+qryAccPeriods.FieldByName('id').ASString);
...
procedure TframeAccPeriods.accEntrysKeyDown(Sender: TObject; var Key: word;
  Shift: TShiftState);
begin
  if TDbGrid(Sender).SelectedRows.Count < 1 then
    TDbGrid(Sender).SelectedRows.CurrentRowSelected := True;

  if (key = VK_DELETE) and (Shift = [ssCtrl]) and
    (TDbGrid(Sender).SelectedRows.Count = 1) and not qryAccPeriods.EOF and
  ...
  begin
    ...
    if Dialogs.messageDlg(estbk_strmsg.SEAccPeriodDeleteRecord,
    ...
    begin
      ...
      //qryAccPeriods.CommitUpdates;
      // ------
      //qryAccPeriods.Refresh;
    end;
    // ---
  end;
end;

procedure TframeAccPeriods.accEntrysSelectEditor(Sender: TObject;
