...
uses
  ...
  estbk_ebill,estbk_frm_accperfinalize,estbk_frm_donations,estbk_frm_template,estbk_lib_ebillimporter,
...
var
   pExAccNrs : AStr;
   pExistingAccRec : Integer;
   tmp : TfrmTemplate;
   test : estbk_lib_ebillimporter.TEbillImporter;
   ptest : TAStrList;
   test3 : AStr;
   pvat : TVatExt2;
   pVatVars : TVatExtVars2;
   pStr : TStringStream;
   pf : TFileStream;
   pcommstatus, pdata, pmessage : AStr;
var
  pFrm : TForm;
  pFra : TFrame;
  pTestId : TADynIntArray;
  ptext : textfile;
  pcoll : TCollection;
  pstr1 : TStringStream;
  ...
  camColl : TCollection;
  bankfrd : TBankFileReader;

  dummydef : TCollection;
  sepafiles: TAStrList;
  vead: Astr;
  prepwriter : TStatusRepWriter;
{$endif}
begin
{$ifdef debugmode}
//  dummydef := TCollection.Create(TBankFileDef);
  prepwriter := TStatusRepWriter.Create(dmodule.primConnection,'SEPAIMP');
  bankfrd := TBankFileReader.Create(prepwriter);
  sepafiles := TAStrList.Create;
  // sepafiles.Add('R:\OsProjects\sepa\EE307700771000685026_Account_Statement_2016-08-25(1).xml');
  // sepafiles.Add('R:\OsProjects\panga_failid\camt_053_kvv-2.xml');
  // sepafiles.Add('R:\OsProjects\panga_failid\camt_052_statement.xml');
  sepafiles.Add('R:\OsProjects\panga_failid\ISO_XML_053_Swed.xml');
  readCam053 := TFI_camt_053.Create;

  bankfrd.readSEPAFiles(sepafiles, 'SEPA', vead);

  FreeAndNil(prepwriter);
  FreeAndNil(bankfrd);
  FreeAndNil(sepafiles);
...

{
  try
    ...
    camFile := TFileStream.Create('R:\OsProjects\sepa\EE307700771000685026_Account_Statement_2016-08-25(1).xml', fmOpenRead);
...
pf := TFileStream.Create('c:\2015-11-30.txt',fmopenread);

pstr.CopyFrom(pf, pf.size);
pstr.seek(0,0);

fin_parseCommonResp(pstr, pcommstatus, pdata, pmessage);
pcoll:=TCollection.Create(TFI_camt_053_item);

pdata := '<?xml version="1.0" encoding="UTF-8"?><Document>' + pdata + '</Document>';
*)
{
memo1.lines.add(pdata);
memo1.lines.SaveToFile('c:\testtest.txt');
}
(*
pstr1:=TStringStream.create(pdata);
TFI_camt_053.ReadCamt053Short(pstr1, pcoll);
pcoll.free;
pstr1.free;
*)
...
TFI_camt_053 class function Read(const pStream : TStringStream;
                        const pColl : TCollection) : Boolean;
}



//pStr := TStringStream.Create('{"status":"ok","data":{"EUR":"57.49"}}');
//{"status":"ok"}
{
pStr := TStringStream.Create('{"status":"ok","data":{"EUR":"57.49"}}');
fin_parseCommonResp(const pJsonStream : TStringStream; var pcommstatus : AStr; var pdata : AStr; var pmessage : AStr);
  pStr.Free;
}
//memo1.lines.Add(uploadFile('http://212.47.212.48/payment/new','c://sepa25112015.xml'));
{
pStr := TStringStream.Create('{"status": "erro", "message": "Error message(i.e. failed to fetch data from bank)" }');
fin_parseBalance(pstr);
pStr.Free;
}
...
(*
 ...
 pTestId[0] := 32;
// pTestId[1] := 33;
 assignfile(ptext,'c:\test.xml');
 rewrite(ptext);
 writeln(ptext, TformSepaPayments.generateSepaPayments('HABAEE2X',estbk_globvars.glob_company_id, ptestid, dmodule.tempQuery));
 closefile(ptext);
*)


{
 with TformSepaPayments.Create(nil, est pTestId) do
 try
   showmodal;
 finally
   free;
...
self._crwndManagerExt(__csmDocumentFilesWnd,
          ...
          TframeDocFilesMgr(pFra).documentId:=875;
          TframeDocFilesMgr(pFra).loadData:=true;
...
(*
 ptest:=TAStrList.Create;
// test3 :=  VatBillExtf(dmodule.tempQuery, 2014,10);
pVatVars:= TVatExtVars2.Create;
 pvat:= TVatExt2.Create(dmodule.tempQuery);


test3 := pvat.VatBillExtf(pVatVars,
              dateutils.StartOfAMonth(yearOf(now),monthOf(now)),
              dateutils.EndOfAMonth(yearOf(now),monthOf(now)),
              yearOf(now),
              monthOf(now));
freeandnil(pVatVars);
...
with dmodule.tempQuery,SQL do
 begin
   {
   ui vastavas perioodis piirmäära ületavad ostu- ja/
või müügitehingud puuduvad, tuleb täita
declarationBody osas elementide noSales ja/või noPu
rchases väärtuseks TRUE. Sellisel juhul
salesAnnex ja/või purchasesAnnex osa ei täideta. Sa
muti tuleb täita noSales ja/või noPurchases
}
(*
   Close;
   Clear;
   Add('SELECT client_id,c.name,c.lastname,c.regnr,bill_type,SUM(ABS(vtlsum)) as blsum
   Add('FROM
   ...
   Add('v.id = b0.vat_id AND v.perc IN (9,20)
   Add('INNER JOIN accounting_register r ON
   Add('r.id = b.accounting_register_id AND r.type_ IN ('P','S','C')
   Add('WHERE b.id = 257 AND b.status NOT LIKE '%D%' AND b.rec_deleted = 'f' AND b.status LIKE '%C%') as bl
   Add('LEFT OUTER JOIN client c ON
   Add('c.id = bl.client_id
   Add('GROUP BY client_id,c.name,c.lastname,c.regnr,bill_type
   Add('HAVING SUM(ABS(vtlsum))>=10
*)
 end;

(*
SELECT client_id,c.name,c.lastname,c.regnr,bill_type,SUM(ABS(vtlsum)) as blsum
FROM
(SELECT  DISTINCT b.bill_number,b.bill_type,b.bill_date,r.transdate,b.client_id,b.credit_bill_id,
 -- b.bill_number,b0.*
(SELECT SUM(a.rec_sum)
FROM accounting_records a
WHERE a.account_id = b0.item_account_id
  AND a.accounting_register_id = r.id
  AND a.rec_deleted = 'f') as vtlsum
FROM bills b
INNER JOIN bill_lines b0 ON
b0.bill_id=b.id
INNER JOIN vat v ON
v.id = b0.vat_id AND v.perc IN (9,20)
INNER JOIN accounting_register r ON
r.id = b.accounting_register_id AND r.type_ IN ('P','S','C')
WHERE b.id = 257 AND b.status NOT LIKE '%D%' AND b.rec_deleted = 'f' AND b.status LIKE '%C%') as bl
LEFT OUTER JOIN client c ON
c.id = bl.client_id
GROUP BY client_id,c.name,c.lastname,c.regnr,bill_type
HAVING SUM(ABS(vtlsum))>=10
...
v.id = b0.vat_id AND v.perc IN (9,20)
INNER JOIN accounting_register r ON
r.type_ IN ('B','P') AND r.rec_deleted = 'f' AND r.id = b.accounting_register_id  AND b.rec_deleted = 'f'
INNER JOIN accounting_records r0 ON
r0.accounting_register_id = r.id AND r0.account_id=b0.item_account_id AND r0.rec_deleted = 'f'
WHERE b.id = 296
*)
// dmodule.__generateClientData('aa00a','sdfsd00fdsfsd','dasdsads','asdasdasd');
{
 test := estbk_lib_ebillimporter.TEbillImporter.Create(nil);
 test.Filename:= 'c:\ebilltest.xml';
 test.Exec;
 test.Free;
}
...
tmp:=TfrmTemplate.create(nil);
tmp.showmodal;
tmp.free;
exit;
//   pExistingAccRec:=0;
//   b:=estbk_acc_perfinalize._generateAccFinalizatingRec(strtodate('20.07.2012'),strtodate('01.01.2012'),strtodate('30.01.2012'),pExAccNrs,pExistingAccRec);
// estbk_frm_accperfinalize.TformAccPerFinalize.showAndCreate;


  // estbk_ebill._generateEBill(24);
  exit;
//self.dataEvents(self,__frameOpenCashRegister,22,Pointer(91));


// --            3587263851
//pFullname:='K.HEINONEN EESTI OÜ';
//estbk_crc32.CRC32(@pFullname[1],length(pFullname),pCRC32);
//showmessage(inttostr(pCRC32));
with dmodule.tempQuery,SQL do
 try
  close;
  clear;
  Add(estbk_sqlclientcollection._SQLStatementOfHolding(1,true));
  open;
  while not eof do
   begin
     showmessage(fieldbyname('ppsum').AsString);
     next;
   end;
 finally
  close;
...
begin
  with TformDonation.Create(nil) do
  try
    showmodal;
  finally
    free;
