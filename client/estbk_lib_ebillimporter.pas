...
type
  ...
  TOnEBillReaderError = procedure(const pTaginfo : AStr;
                                  const pError : AStr;
                                  const pErrorLevel : TCEBillReaderErrorLevel;
                                  var pResume : Boolean) of object;

  TOnEBillProcessingUIMod = procedure(const pInvoiceNr : AStr;
...
type
  ...
  protected
   ...
   procedure ProcessData(const pInvoiceItemCnt : Integer;
                         const pInvoiceType : AStr;
                         const pInvoiceDoc : AStr;
                         const pInvoiceNumber : AStr;
                         const pInvoiceContentText : AStr;
                         const pInvoiceDate : AStr;
                         const pDueDate : AStr;
                         const pTotalSum : Double;
                         const pInvoiceSum : Double;
                         const pCurrency : AStr;
                         const pObjSellerParty : TDomNode;
                         const pObjBuyerParty : TDomNode;
                         const pObjPaymentInfo : TDomNode;
                         const pBillItems : TObjectList;
                         const pVATSummaryItems : TObjectList);
  public
   ...
   property OnEBillProcessingUIModEvent : TOnEBillProcessingUIMod read FEBillProcessingModEvent write FEBillProcessingModEvent; // üldiselt teeb tagataustal arve frame, edastab andmed ja loodab parimat !
   function Exec : Boolean;
   constructor Create(const pReaderErrorEvent : TOnEBillReaderError;
                      const pEBillProcessingUIModEvent : TOnEBillProcessingUIMod = nil);reintroduce;
...
constructor TEbillImporter.Create(const pReaderErrorEvent : TOnEBillReaderError;
                                  const pEBillProcessingUIModEvent : TOnEBillProcessingUIMod = nil);
begin
  inherited Create;
...
procedure TEbillImporter.ProcessData(const pInvoiceItemCnt : Integer;
                                     const pInvoiceType : AStr;
                                     const pInvoiceDoc : AStr;
                                     const pInvoiceNumber : AStr;
                                     const pInvoiceContentText : AStr;
...
begin
  ...
  try
   ...
   if assigned(self.OnEBillProcessingUIModEvent) then
      self.OnEBillProcessingUIModEvent(pInvoiceNumber, pInvoiceDateCnv, pDueDateCnv, pCustomername, pCustomerregCode, pCustomerVAT,  pVATSummaryItems, pBillItems);
...
var
    Doc: TXMLDocument;
    node,node1,node2,node3,Root,Header,Footer,Invoice,
    ...
    VAT, Item,TextNode,LegalAddress,MailAddress,PaymentInfo: TDOMNode;
    pInvoiceItemCnt,i,j,k,z,w,x : Integer;
    nodename,val : AStr;
    pInvoiceType, pInvoiceDoc,pInvoiceNumber,
    pInvoiceContentText, pInvoiceDate, pDueDate, pCurrency : AStr;
    ...
    begin
      ...
      if  nodename = 'invoice' then
        begin // @@INVOICE
            ...
            pInvoiceContentText := ''; // <InvoiceContentText>LENNUPILET/ID - FLUGSCHEIN/E X8YXGE Lufthansa Doris Scholz / Helmut Weiss   		 Flug 1 	Samstag, 28.</InvoiceContentText>
          ...
          if nodename = 'invoiceparties' then
            ...
            *** hetkel ignoreerime !
            ...
            begin
              ...
              <PaymentDescription>13518001</PaymentDescription>
              <Payable>YES</Payable>
              <PayDueDate>2014-02-21</PayDueDate>
              <PaymentTotalSum>618</PaymentTotalSum>
              <PayerName>Helmut Weiß</PayerName>
              <PaymentId>2072248</PaymentId>
              <PayToAccount>EE142200221058044017</PayToAccount>
              <PayToName>Chalaro OÜ</PayToName>
            end; *)
        end;
