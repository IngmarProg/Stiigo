inherited frameDebtors: TframeDebtors
  Height = 223
  Width = 618
  ClientHeight = 223
  ClientWidth = 618
  DesignLeft = 189
  DesignTop = 358
  object grpDebt: TGroupBox[0]
    Left = 8
    Height = 218
    Top = 0
    Width = 609
    Anchors = [akTop, akLeft, akRight, akBottom]
    Caption = ' Tasumata arved '
    ClientHeight = 200
    ClientWidth = 605
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    object lblCustId: TLabel
      Left = 6
      Height = 14
      Top = 10
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Klient/Hankija id:'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object edtClientCode: TEdit
      Left = 115
      Height = 21
      Top = 7
      Width = 95
      MaxLength = 10
      OnChange = edtClientCodeChange
      OnExit = edtClientCodeExit
      OnKeyDown = edtClientCodeKeyDown
      OnKeyPress = edtClientCodeKeyPress
      TabOrder = 0
    end
    object btnOpenClientList: TBitBtn
      Left = 211
      Height = 21
      Top = 7
      Width = 16
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
      }
      OnClick = btnOpenClientListClick
      TabOrder = 1
    end
    object edtCustNameAndRegCode: TEdit
      Tag = -1
      Left = 230
      Height = 21
      Top = 7
      Width = 255
      Color = cl3DLight
      ReadOnly = True
      TabStop = False
      TabOrder = 12
    end
    object btnClean: TBitBtn
      Tag = -1
      Left = 496
      Height = 30
      Top = 35
      Width = 80
      Caption = 'Puhasta'
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
      }
      OnClick = btnCleanClick
      TabOrder = 11
    end
    object lblBillsuntil: TLabel
      Left = 6
      Height = 14
      Top = 33
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Arved seisuga:'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object dtBillsUntil: TDateEdit
      Left = 115
      Height = 21
      Top = 30
      Width = 95
      CalendarDisplaySettings = [dsShowHeadings, dsShowDayNames]
      OKCaption = 'OK'
      CancelCaption = 'Cancel'
      DateOrder = doNone
      ButtonWidth = 23
      CharCase = ecNormal
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
      }
      NumGlyphs = 0
      MaxLength = 0
      OnChange = dtBillsUntilChange
      OnExit = dtBillsUntilExit
      OnKeyPress = edtClientCodeKeyPress
      TabOrder = 2
    end
    object lblOverDDate: TLabel
      Left = 240
      Height = 14
      Top = 33
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Maksetähtaeg üle:'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object edtDDaysOver: TEdit
      Left = 347
      Height = 21
      Top = 30
      Width = 48
      MaxLength = 4
      OnKeyPress = edtDDaysOverKeyPress
      TabOrder = 3
    end
    object lblDaysOvr: TLabel
      Left = 400
      Height = 14
      Top = 33
      Width = 36
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'päeva'
      ParentColor = False
    end
    object pSort: TLabel
      Left = 6
      Height = 14
      Top = 120
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Aruande järjestus:'
      ParentColor = False
    end
    object cmbSortByFields: TComboBox
      Left = 115
      Height = 21
      Top = 117
      Width = 280
      AutoSize = False
      ItemHeight = 13
      OnChange = cmbSortByFieldsChange
      OnKeyPress = edtClientCodeKeyPress
      Style = csDropDownList
      TabOrder = 8
    end
    object pchkboxSortOrder: TCheckBox
      Left = 409
      Height = 17
      Top = 120
      Width = 38
      Caption = 'A-Z'
      Checked = True
      OnKeyPress = edtClientCodeKeyPress
      State = cbChecked
      TabOrder = 9
    end
    object btnReport: TBitBtn
      Tag = -1
      Left = 496
      Height = 30
      Top = 3
      Width = 80
      Caption = 'Aruanne'
      Glyph.Data = {
        
        4DFEDDAF8DC1FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
      }
      OnClick = btnReportClick
      TabOrder = 10
    end
    object lblBillCreatedBy: TLabel
      Left = 6
      Height = 14
      Top = 56
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Arve koostas:'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object cmbBillCreatedby: TComboBox
      Left = 115
      Height = 21
      Top = 53
      Width = 280
      AutoDropDown = True
      AutoSize = False
      ItemHeight = 13
      OnKeyPress = edtClientCodeKeyPress
      Style = csDropDownList
      TabOrder = 4
    end
    object lblCurrency: TLabel
      Left = 6
      Height = 14
      Top = 148
      Width = 106
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Valuuta:'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object cmbCurrencyList: TComboBox
      Left = 115
      Height = 21
      Top = 144
      Width = 54
      ItemHeight = 13
      OnChange = cmbCurrencyListChange
      OnKeyPress = edtClientCodeKeyPress
      Style = csDropDownList
      TabOrder = 5
      Visible = False
    end
    object chkCurrencyDiff: TCheckBox
      Left = 177
      Height = 17
      Top = 147
      Width = 123
      Caption = 'Arvuta kursivahed'
      Enabled = False
      OnChange = chkCurrencyDiffChange
      OnKeyPress = edtClientCodeKeyPress
      TabOrder = 6
      Visible = False
    end
    object chkBoxPartiallyPaid: TCheckBox
      Left = 115
      Height = 17
      Top = 99
      Width = 175
      Caption = 'Vaid osaliselt tasutud arved'
      OnKeyPress = edtClientCodeKeyPress
      TabOrder = 7
    end
    object btnOutput: TBitBtn
      Left = 575
      Height = 21
      Top = 7
      Width = 16
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
      }
      OnClick = btnOutputClick
      TabOrder = 13
    end
  end
  object qryDebtBills: TZReadOnlyQuery[1]
    Params = <>
    left = 40
    top = 8
  end
  object report: TfrReport[2]
    Dataset = reportdata
    InitialZoom = pzDefault
    Options = []
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit]
    DataType = dtDataSet
    OnBeginDoc = reportBeginDoc
    OnBeginBand = reportBeginBand
    OnGetValue = reportGetValue
    OnEnterRect = reportEnterRect
    left = 456
    top = 8
    ReportForm = {
      19000000
    }
  end
  object reportdata: TfrDBDataSet[3]
    DataSet = qryDebtBills
    left = 400
    top = 8
  end
  object popupReportType: TPopupMenu[4]
    left = 104
    top = 8
    object mnuItemScreen: TMenuItem
      Caption = 'Ekraanile'
      Checked = True
      OnClick = mnuItemScreenClick
    end
    object mnuItemPdfFile: TMenuItem
      Caption = 'PDF faili'
      OnClick = mnuItemScreenClick
    end
    object mnuItemCsv: TMenuItem
      Caption = 'CSV faili'
      OnClick = mnuItemScreenClick
    end
  end
  object saveRep: TSaveDialog[5]
    Title = 'Salvestame'
    DefaultExt = '.*.pdf'
    left = 152
    top = 8
  end
end
