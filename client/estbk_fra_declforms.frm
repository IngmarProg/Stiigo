object frameDeclForms: TframeDeclForms
  Left = 0
  Height = 488
  Top = 0
  Width = 699
  ClientHeight = 488
  ClientWidth = 699
  OnResize = FrameResize
  TabOrder = 0
  DesignLeft = 256
  DesignTop = 217
  object grpBoxDefDeclLines: TGroupBox
    Left = 6
    Height = 418
    Top = 4
    Width = 690
    Anchors = [akTop, akLeft, akRight, akBottom]
    Caption = ' Deklaratsioon'
    ClientHeight = 400
    ClientWidth = 686
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    object pgridDeclForms: TStringGrid
      Left = 7
      Height = 393
      Top = 1
      Width = 672
      Anchors = [akTop, akLeft, akRight, akBottom]
      AutoEdit = False
      ColCount = 6
      Columns = <      
        item
          Title.Caption = 'Tunnus'
          Width = 45
        end      
        item
          MaxSize = 0
          ReadOnly = True
          Title.Caption = 'Rea nimetus'
          Width = 327
        end      
        item
          SizePriority = 0
          Title.Caption = 'D.nr'
          Width = 65
        end      
        item
          ButtonStyle = cbsEllipsis
          Title.Caption = 'Rea valem'
          Width = 280
        end      
        item
          MinSize = 28
          MaxSize = 28
          ReadOnly = True
          SizePriority = 0
          Title.Caption = ' '
          Width = 28
        end      
        item
          MinSize = 1
          MaxSize = 1
          ReadOnly = True
          Title.Caption = ' '
          Width = 0
          Visible = False
        end>
      DefaultColWidth = 24
      DefaultRowHeight = 22
      FixedCols = 0
      Flat = True
      Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goColSizing, goEditing, goRowSelect, goAlwaysShowEditor, goSmoothScroll]
      ParentFont = False
      RowCount = 1000
      ScrollBars = ssVertical
      TabOrder = 0
      OnClick = pgridDeclFormsClick
      OnDblClick = pgridDeclFormsDblClick
      OnDrawCell = pgridDeclFormsDrawCell
      OnEditButtonClick = pgridDeclFormsEditButtonClick
      OnEditingDone = pgridDeclFormsEditingDone
      OnExit = pgridDeclFormsExit
      OnKeyDown = pgridDeclFormsKeyDown
      OnPrepareCanvas = pgridDeclFormsPrepareCanvas
      OnResize = pgridDeclFormsResize
      OnSelectEditor = pgridDeclFormsSelectEditor
      OnSelectCell = pgridDeclFormsSelectCell
      OnShowHint = pgridDeclFormsShowHint
      ColWidths = (
        45
        327
        65
        280
        28
        0
      )
    end
  end
  object btnSave: TBitBtn
    Tag = -1
    Left = 352
    Height = 30
    Top = 443
    Width = 95
    Anchors = [akRight, akBottom]
    Caption = 'Salvesta'
    Enabled = False
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00
    }
    OnClick = btnSaveClick
    TabOrder = 1
  end
  object btnCancel: TBitBtn
    Tag = -1
    Left = 448
    Height = 30
    Top = 443
    Width = 95
    Anchors = [akRight, akBottom]
    Caption = 'Katkesta'
    Enabled = False
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FAFAFF00FFFFFF00FFFFFF00FFFFFF00
    }
    OnClick = btnCancelClick
    TabOrder = 2
  end
  object btnClose: TBitBtn
    Tag = -1
    Left = 592
    Height = 30
    Top = 443
    Width = 95
    Anchors = [akRight, akBottom]
    Caption = 'Sulge'
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF006D9CD4896A9AD2FB6697CFEE
    }
    OnClick = btnCloseClick
    TabOrder = 3
  end
  object qryTaxFormsSel: TZQuery
    Params = <>
    left = 624
    top = 16
  end
  object qryWorker: TZQuery
    Params = <>
    left = 528
    top = 16
  end
  object pSpecialTasks: TPopupMenu
    left = 448
    top = 16
    object pnewForm: TMenuItem
      Caption = 'Lisa uus käibemaksu vorm'
      OnClick = pnewFormClick
    end
  end
  object qryFormdSeq: TZSequence
    SequenceName = 'formd_id_seq'
    left = 376
    top = 16
  end
  object pFormdLinesEventsPopup: TPopupMenu
    left = 624
    top = 80
    object mnuItemNewLine: TMenuItem
      Caption = 'Uus rida'
      OnClick = mnuItemNewLineClick
    end
    object mnuSep: TMenuItem
      Caption = '-'
    end
    object mnuItemUp: TMenuItem
      Caption = 'Liiguta rida üles'
      OnClick = mnuItemUpClick
    end
    object mnuItemDown: TMenuItem
      Caption = 'Liiguta rida alla'
      OnClick = mnuItemDownClick
    end
    object mnu2Spaces: TMenuItem
      Caption = 'Lisa rea nimetuse ette 3 tühikut'
      OnClick = mnu2SpacesClick
    end
    object mnuItem: TMenuItem
      Caption = '-'
    end
    object mnuCopyCell: TMenuItem
      Caption = 'Kopeeri lahtri sisu'
      OnClick = mnuCopyCellClick
    end
    object mnuPaste: TMenuItem
      Caption = 'Paiguta lahtri sisu'
      OnClick = mnuPasteClick
    end
    object mnuSep3: TMenuItem
      Caption = '-'
    end
    object mnuItemImport: TMenuItem
      Caption = 'Impordi definitsioonid'
      OnClick = mnuItemImportClick
    end
    object mnuItemExport: TMenuItem
      Caption = 'Ekspordi definitsioonid'
      OnClick = mnuItemExportClick
    end
  end
  object pSaveXmlAs: TSaveDialog
    Title = 'Salvesta kui'
    DefaultExt = '.xml'
    Filter = 'Xml fail|*.xml|Kõik failid|*.*'
    left = 24
    top = 24
  end
  object pOpenXmlAs: TOpenDialog
    Title = 'Laeme faili...'
    DefaultExt = '.xml'
    Filter = 'Xml failid|*.xml|Kõik failid|*.*'
    left = 112
    top = 24
  end
end
