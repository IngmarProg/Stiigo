...
//{$else}
const CBillSumPer = 1000; // eurot arved  -- hardcoded !?!
//{$endif}
...
type
  ...
  protected
    ...
    FEUSupplyInclGoodsAndServicesZeroVat : Double; //  (KMD rida 3.1) Kauba ühendusesisene käive ja teise liikmesriigi maksukohustuslasele/piiratud maksukohustuslasele osutatud teenuste käive kokku
    ...
    FEUAcquisitionsGoodsAndServicesTotal : Double; //  (KMD rida 6) Kauba ühendusesisene soetamine ja teise liikmesriigi maksukohustuslaselt saadud teenused kokku
    ...
    FAcquisitionImmovablesAndScrapMetalAndGold : Double; // Erikorra alusel maksustatava kinnisasja, metallijäätmete ja väärismetalli  soetamine(KMS § 41^1) (KMD rida 7.1)
    FSupplyExemptFromTax : Double; // Maksuvaba käive (KMD rida 8)
    FSupplySpecialArrangements : Double; // (KMD rida 9) Erikorra alusel maksustatava kinnisasja, metallijäätmete ja väärismetalli  käive (KMS § 41^1) ning teises liikmesriigis
  ...
  published
    ...
    property p7x_AcquisitionOtherGoodsAndServicesTotal : Double read FAcquisitionOtherGoodsAndServicesTotal write FAcquisitionOtherGoodsAndServicesTotal;
    property p71_AcquisitionImmovablesAndScrapMetalAndGold : Double read FAcquisitionImmovablesAndScrapMetalAndGold write FAcquisitionImmovablesAndScrapMetalAndGold;
...
type
  ...
  public
    ...
    // kunagi kui sul tõesti midagi muud teha pole ja aega ka nii nagu raba, vii ka DOM objekti peale üle
    function VatBillExtf(const pVatVars : TVatExtVars2;
                         const pVatPerStart : TDatetime;
                         const pVatPerEnd : TDatetime;
                         const pYear : Integer; // millise kuu ja aasta käibekaga tegemist !
                         const pMonth : Integer
                         ) : AStr;
  end;
...
function TVatExt2.VatBillExtf(const pVatVars : TVatExtVars2;
                              const pVatPerStart : TDatetime;
                              const pVatPerEnd : TDatetime;
                              const pYear : Integer;
                              const pMonth : Integer
...
function _buildQueryDetailed(const pType : AStr = '';
                             const pCustlist : TList = nil;
                             const pGroupAlsoPercData : Boolean = false;
                             const pBillIdlist : TList = nil) : AStr;
var
  i : Integer;
...
begin
  ...
  with pQStrList do
   begin
     ...
     Add('SELECT client_id,c.name,c.lastname,c.regnr,bl.bill_number,bl.bill_date,bl.credit_bill_id ' +
           IfThen(pGrpAlsoByVat, ',v.perc') +

           ',SUM(vtlsum) + MAX(roundsum) as blsum,' +  // meil arveread ju kontode kaupa grupeeritud, tuleb kokku lüüa;
           ' MAX(vatsum) as vsum'); // aga käibemaksu summa küsitakse konkreetselt arvelt endalt; mitte SUM vaid MAX krdi pudru ja kapsad; SUM(vatsum) as vsum
     Add('FROM');


     Add('(SELECT  DISTINCT b.bill_number,b.bill_type,b.bill_date,r.transdate,'+
         'b.client_id,b.credit_bill_id,v.id as vat_id,b.roundsum,');
 ...
 if (pType <> '') and (billstrl = '') then
     ...
     Add(format('  AND b.bill_date BETWEEN %s AND %s',[quotedStr(formatdatetime('yyyy-mm-dd',pVatPerStart)), quotedStr(formatdatetime('yyyy-mm-dd',pVatPerEnd))]));
   ...
   *)
   ...
   with ps do
    begin
     ...
     add('<declarationType>1</declarationType>'); // Kohustuslik. KMD ja KMD INF’i esitamine tava või pankrotiperioodi kohta. Väärtused 1-2. 1 - Tavaperiood, 2 - Pankrotiperiood
...
*)
     ...
     *)
     add('<acquisitionOtherGoodsAndServicesTotal>' + _xf(pVatVars.p7x_AcquisitionOtherGoodsAndServicesTotal) + '</acquisitionOtherGoodsAndServicesTotal>');

     (*
     Erikorra alusel maksustatava kinnisasja, metallijäätmete ja väärismetalli  soetamine(KMS § 41^1) (KMD rida 7.1)
     *)
     add('<acquisitionImmovablesAndScrapMetalAndGold>' + _xf(pVatVars.p71_AcquisitionImmovablesAndScrapMetalAndGold) + '</acquisitionImmovablesAndScrapMetalAndGold>');
