(*
  ...
  Typhon-FPC-LINUX codeset conversion routines 
  ...
  and call LocalizeForm('TFormClassName') right after {$I TFormClassName.ctrs} - 
  see example below. 
  ...
  1. Auto form conversion 
     ...
     - add LocalizeForm('TFormClassName') after {$I TFormClassName.ctrs} 
     <CODE> 
       unit1; 
       [...] 
       initialization 
...
begin 
    ...
    {$ENDIF FORCE_UTF8} 
    {find resource} 
    res:=TyphonResources.Find(form_classname); 
...
var
  i:integer;
begin
  for i:=0 to TyphonResources.Count - 1 do
  begin
    if TyphonResources.Items[i].ValueType = 'FORMDATA' then
      LocalizeForm(TyphonResources.Items[i].Name);
  end;
end;
...
{$ENDIF LCLGtk2} 
finalization 
//  iconv_close(ic_localize); // ct9999 SOS give error on IDE rebuild on Solaris and FreeBSD
{.$IFDEF LCLGtk2} 
//  iconv_close(ic_str2gtk);  // ct9999 SOS give error on IDE rebuild on Solaris and FreeBSD
//  iconv_close(ic_gtk2str);  // ct9999 SOS give error on IDE rebuild on Solaris and FreeBSD
{.$ENDIF LCLGtk2} 
{$ENDIF}
*)
end.
