...
type
  ...
  private
    ...
    // 08.08.2010 Ingmar
    function GetLookupTitleColClick:TOnLookupTitleColClick;
    procedure SetLookupTitleColClick(const AValue: TOnLookupTitleColClick );
    procedure SetPopUpVisible(const v : Boolean);
  ...
  protected
    ...
    property OnClosePopupNotif:TClosePopup read FOnClosePopup write FOnClosePopup;
  ...
  public
    ...
    // 21.11.2009 ingmar; tahad ka write, et saaksin sundida listi kuvama !
    property PopupVisible:boolean read GetPopupVisible write SetPopUpVisible;
    property OnLookupTitleColClick : TOnLookupTitleColClick read GetLookupTitleColClick write SetLookupTitleColClick;
  end;
...
// 28.05.2011 Ingmar
procedure TRxCustomDBLookupCombo.SetLookupTitleColClick(
  const AValue: TOnLookupTitleColClick );
begin
  FPopUpFormOptions.OnLookupTitleColClick:=AValue;
end;

// 21.11.2009 Ingmar
procedure TRxCustomDBLookupCombo.SetPopUpVisible(const v : Boolean);
begin
 //if not v then
 //   self.HideList
 //else
  if not assigned(self.FRxPopUpForm) then
    ShowList;
end;
...
{    if Key=VK_RETURN then HideList
  ...
  // 20.11.2009 Ingmar
   if  ((Key = VK_DOWN) and ((ssAlt in Shift) or (ssCtrl in Shift)))
    or ((Key = VK_RETURN) and (Shift=[ssCtrl])) then
   // or ((Key=VK_CONTROL)  and (Shift=[ssCtrl]))  then
   begin
     ShowList;
     Key := 0;
   end
...
// 29.05.2011 Ingmar
function TRxCustomDBLookupCombo.GetLookupTitleColClick:TOnLookupTitleColClick;
begin
  Result:=FPopUpFormOptions.OnLookupTitleColClick;
end;
